...
function addingOptions() {
  fetchBreeds()
    .then(catsList => {
      ...
      const defaultOption =
        '<option value="default">Оберіть породу кота</option>';
      selectCat.innerHTML =
        defaultOption +
        catsList.data
...
function addingInformation(evt) {
  ...
  if (idCat !== 'default') {
    // Порівнюємо з рядком "default"
    const defOpt = selectCat.querySelector('option[value="default"]');
    if (defOpt) {
      defOpt.remove();
      //не можу зрозуміти як видаляти дефолтну опцію вибору
    }
